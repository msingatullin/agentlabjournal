...
def fallback_html() -> str:
    ...
    stylesheets = f'<link rel="stylesheet" href="{stylesheet_prefix}style.css"><link rel="stylesheet" href="{stylesheet_prefix}reading.css">'
    if args.language == "en":
        ...
        sections = [("Test boundary", f"The test addresses {args.problem.lower()}. It is limited to the stated scenario and does not claim production reliability."), ("Minimal scenario", f"Define one repeatable test, keep the input and model settings stable, and record each run with a stable identifier. The expected result is: {args.result}."), ("Verification", "Run the same cases several times, save the measured outputs and compare the result against the acceptance criteria. Do not replace measurements with a model-generated conclusion."), ("Limitations", "This is a reproducible field test, not a security certification or a guarantee of production behavior.")]
    else:
        title = f"{args.title}: {seo_passport['primary_query']}"
        lead = f"Этот практический материал описывает {args.problem.lower()} и задаёт воспроизводимый тест по теме «{seo_passport['primary_query']}» без неподтверждённых обещаний результата."
        sections = [("Граница теста", f"Проверяем только сценарий: {args.problem.lower()}. Материал не утверждает готовность контура к production."), ("Минимальный сценарий", f"Зафиксируйте входные данные, настройки модели и критерий приёмки. Ожидаемый результат: {args.result}. Каждому прогону присвойте стабильный идентификатор."), ("Проверка", "Повторите тест на одинаковом наборе, сохраните измерения и сравните их с критерием. Не подменяйте измеренные данные выводом модели."), ("Ограничения", "Это воспроизводимый учебный сценарий, а не сертификация безопасности и не гарантия поведения в production.")]
    body = "".join(f"<h2>{h}</h2><p>{t}</p>" for h,t in sections)
    related = f"This material uses the measured query cluster: {', '.join(seo_queries)}. It does not promise a search result." if args.language == "en" else f"Материал использует измеренные запросы: {', '.join(seo_queries)}. Он не обещает поискового результата."
...
if "rel=\"stylesheet\"" not in html:
    html = html.replace("</head>", f'<link rel="stylesheet" href="{stylesheet_prefix}style.css"><link rel="stylesheet" href="{stylesheet_prefix}reading.css"></head>', 1)
